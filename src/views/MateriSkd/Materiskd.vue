    ...
    <div v-if="viewMode === 'discovery'" class="animate-in fade-in duration-700">
      ...
      <nav class="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 py-4">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
          <div class="flex items-center gap-3">
            <RouterLink
              to="/dashboard"
              class="h-10 w-10 flex items-center justify-center rounded-xl bg-emerald-600 text-white shadow-lg shadow-emerald-200 hover:scale-105 transition-transform"
            >
          ...
          <div class="hidden md:block relative w-96">
            ...
            />
            <svg
              class="absolute left-4 top-3 h-5 w-5 text-slate-400"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
      ...
      <header class="relative overflow-hidden bg-slate-900 px-6 py-16 sm:py-24">
        <div class="absolute inset-0 opacity-20">
          <div
            class="absolute top-0 -left-4 w-72 h-72 bg-emerald-500 rounded-full mix-blend-multiply filter blur-3xl animate-blob"
          ></div>
          <div
            class="absolute top-0 -right-4 w-72 h-72 bg-blue-500 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-2000"
          ></div>
          <div
            class="absolute -bottom-8 left-20 w-72 h-72 bg-purple-500 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-4000"
          ></div>
        </div>
        ...
        <div class="relative max-w-7xl mx-auto text-center">
          ...
          <p class="text-slate-400 text-lg max-w-2xl mx-auto font-medium">
            Pilih modul pembelajaran yang dirancang khusus oleh mentor berpengalaman untuk membantumu lolos seleksi CPNS
            2024.
          </p>
        </div>
      </header>
        ...
        <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 p-4 border border-slate-100">
          <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
              ...
              <select
                class="bg-slate-50 border-none rounded-xl text-xs font-bold text-slate-700 focus:ring-emerald-500/20 px-4 py-2"
              >
          ...
          >
            ...
            <div class="relative aspect-[16/10] overflow-hidden bg-slate-100">
              ...
              <div class="absolute top-4 left-4 flex gap-2">
                <span
                  class="px-2.5 py-1 rounded-lg bg-white/20 backdrop-blur-md text-[10px] font-black text-white uppercase tracking-widest border border-white/20"
                >
                  {{ module.category }}
                </span>
                <span
            ...
            <div class="p-8 flex-1 flex flex-col">
              <h3
                class="text-xl font-black text-slate-800 mb-3 group-hover:text-emerald-600 transition-colors leading-snug"
              >
                {{ module.title }}
              </h3>
              ...
              <div class="mt-auto flex gap-3">
                ...
                <button
                  class="h-12 w-12 flex items-center justify-center rounded-2xl bg-slate-50 text-slate-500 hover:bg-slate-100 transition-colors"
                >
      ...
      <footer class="max-w-7xl mx-auto px-6 py-20">
        <div
          class="bg-gradient-to-r from-emerald-600 to-teal-700 rounded-[3rem] p-12 text-center text-white relative overflow-hidden shadow-2xl shadow-emerald-200"
        >
          <div
          ...
          <p class="text-emerald-50 mb-8 max-w-xl mx-auto font-medium opacity-80">
            Gunakan fitur rekomendasi belajar kami untuk mendapatkan kurikulum yang dipersonalisasi sesuai kelemahanmu.
          </p>
          <button
            class="px-10 py-4 bg-white text-emerald-700 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:scale-105 transition-transform"
          >
    ...
    <div v-else class="h-screen flex overflow-hidden animate-in slide-in-from-right duration-500">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-sm font-black text-slate-800 uppercase tracking-tighter">
                {{ selectedModule?.title || "Learning Module" }}
              </h2>
      ...
      <main class="flex-1 bg-white overflow-y-auto">
        <div v-if="selectedMateri" class="max-w-[900px] mx-auto p-10 space-y-8 animate-in fade-in duration-500">
          ...
          <div
            v-if="selectedMateri.type === 'Video'"
            class="group relative aspect-video overflow-hidden rounded-[40px] bg-slate-900 shadow-2xl"
          >
            <div class="absolute inset-0 flex flex-col items-center justify-center bg-slate-900/40 backdrop-blur-sm">
              <button
                class="h-20 w-20 flex items-center justify-center rounded-full bg-white text-emerald-600 shadow-xl shadow-slate-900/50 hover:scale-110 transition-transform"
              >
                <svg class="ml-1 h-10 w-10" fill="currentColor" viewBox="0 0 20 20">
                  <path
                    d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z"
                  />
                </svg>
              </button>
              <p class="mt-6 text-white text-xs font-black uppercase tracking-[0.4em] opacity-80">
                Video Sesi {{ selectedMateri.id }}
              </p>
            </div>
          </div>
          ...
          <div class="bg-white rounded-[40px] border border-slate-100 shadow-xl overflow-hidden">
            <div class="flex border-b border-slate-100 bg-slate-50/30 p-2 gap-2">
              <button
                v-for="t in ['Materi', 'Pembahasan', 'Referensi']"
                :key="t"
                @click="activeTab = t"
                :class="[
              ...
              <div v-if="activeTab === 'Materi'" class="prose prose-slate max-w-none animate-in fade-in">
                ...
                <div class="mt-10 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div
                    v-for="i in 4"
                    :key="i"
                    class="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex gap-4 items-center"
                  >
                    <div
                      class="h-10 w-10 shrink-0 flex items-center justify-center rounded-xl bg-white text-emerald-600 text-lg"
                    >
                      🔑
                    </div>
                    <p class="text-sm font-bold text-slate-700">
                      Strategi Kunci & Point Penting {{ i }} untuk {{ activeCategory }}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div v-for="q in 2" :key="q" class="p-6 rounded-3xl border border-slate-200">
                  ...
                  <div class="p-4 rounded-2xl bg-amber-50 border border-amber-100">
                    <p class="text-sm font-medium text-amber-900 leading-relaxed">
                      Analisis: {{ selectedMateri.title }} sering kali berfokus pada penalaran
                      {{ activeCategory === "TIU" ? "logika" : "konteks" }}...
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div class="flex items-center justify-between pb-10">
            <button
              @click="prevLesson"
              :disabled="!canPrev"
              class="px-8 py-4 rounded-3xl border border-slate-200 bg-white text-slate-600 font-black text-[10px] uppercase tracking-widest disabled:opacity-20 transition-all hover:bg-slate-50"
            >
              ← Sebelumnya
            </button>
            <button
              @click="nextLesson"
              :disabled="!canNext"
              class="px-10 py-4 rounded-3xl bg-emerald-600 text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-emerald-200 disabled:opacity-20 transition-all hover:bg-emerald-700"
            >
              Selanjutnya →
            </button>
          </div>
        ...
        <div v-else class="h-full flex flex-col items-center justify-center text-center p-10">
          ...
          <p class="mt-2 text-slate-400 font-bold uppercase tracking-widest text-[10px]">
            Silakan pilih sub-materi di samping halaman
          </p>
        </div>
      </main>
    </div>
...
const courseModules = ref([
  {
    ...
    image: "https://picsum.photos/seed/twk/800/600"
  },
  {
    id: 2,
    ...
    image: "https://picsum.photos/seed/tiu/800/600"
  },
  {
    id: 3,
    ...
    image: "https://picsum.photos/seed/tkp/800/600"
  }
])
...
const materiList = ref([
  {
    id: 1,
    title: "Pancasilaku: Sejarah & Perumusan",
    category: "TWK",
    type: "Video",
    duration: 15,
    completed: true,
    content:
      "Pancasila merupakan ideologi negara yang lahir dari pemikiran luhur para pendiri bangsa. Sesi ini akan membongkar sejarah Sidang BPUPKI secara mendalam."
  },
  {
    id: 2,
    title: "UUD 1945 & Dinamika Amendemen",
    category: "TWK",
    type: "Materi",
    duration: 25,
    completed: false,
    content:
      "Konstitusi kita telah mengalami 4 kali perubahan. Memahami apa yang berubah dan mengapa adalah kunci menjawab soal integritas."
  },
  {
    id: 3,
    title: "Silogisme: Logika Tanpa Cacat",
    category: "TIU",
    type: "Video",
    duration: 20,
    completed: false,
    content:
      "Trik cepat menarik kesimpulan dari premis mayor dan minor tanpa harus menggali terlalu dalam ke filsafat logika."
  },
  {
    id: 4,
    title: "Aljabar Cepat & Perbandingan",
    category: "TIU",
    type: "Video",
    duration: 15,
    completed: false,
    content: "Menghitung persentase dan perbandingan senilai/berbalik nilai dalam hitungan detik tanpa kertas coretan."
  },
  {
    id: 5,
    title: "Etika & Integritas Kerja ASN",
    category: "TKP",
    type: "Materi",
    duration: 18,
    completed: false,
    content:
      "Bagaimana cara seorang ASN bersikap saat menghadapi konflik kepentingan? Kita bahas di modul profesionalisme ini."
  }
])
