<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div class="flex-1" v-else>
        ...
        >
          ...
          <div class="grid grid-cols-3 gap-2 sm:gap-3 text-xs sm:text-sm">
            <div
              class="rounded-xl bg-white border border-slate-200 shadow-sm hover:shadow-md transition px-3 py-2"
            >
        ...
        <section class="space-y-3">
          <h2 class="text-sm font-semibold text-slate-700">
            ...
            <span class="text-xs font-normal text-slate-500"
              >• {{ activeTryouts.length }} paket</span
            >
          </h2>
    ...
    >
      <div
        class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl text-center animate-scaleIn"
      >
        <div
          class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100 mb-4"
        >
          <svg
        ...
        <p class="mt-2 text-sm text-slate-500">
          Berhasil mengirim bukti persyaratan ikut tryout dan sudah bisa ikut
          tryout.
        </p>
        <button
...
const nextSuggestion = computed(() => {
  ...
  const inProgress = activeTryouts.value.find((t) =>
  ...
  const lowestScoreList = [...activeTryouts.value]
    ...
    .sort(
      (a, b) => (Number(a.bestScore) || 9999) - (Number(b.bestScore) || 9999),
    );
  ...
  if (notStarted) {
    return {
      ...notStarted,
      reason:
        "Kamu punya tryout yang belum dikerjakan, yuk mulai latihan sekarang!",
    };
  }
